...
''' Previous decoder layer with context'''
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, num_layers, d_model, heads, d_ff, dropout, embeddings, tag_embeddings=None):
    ...
    def forward(self, tgt, memory_bank, state, tag=None, memory_lengths=None,
                step=None, cache=None,memory_masks=None, decoding_mask=None):
        """
        ...
        if self.tag_embeddings is not None:
            emb = emb + self.tag_embeddings(tag)
        ...
        if (not memory_masks is None):
            # tag mask
            assert tag.size(1) == 1
            src_tag_mask = memory_masks[torch.arange(memory_masks.size(0)).unsqueeze(1),
                           torch.arange(memory_masks.size(1)).unsqueeze(0), tag-1]
            memory_masks = src_tag_mask.unsqueeze(1) == 0
            # mem mask to pad mask
        ...
        # if not decoding_mask is None:
        #     tgt_tag_mask = (decoding_mask != tag).unsqueeze(1)
        #     tgt_pad_mask = (tgt_pad_mask + tgt_tag_mask) > 0
